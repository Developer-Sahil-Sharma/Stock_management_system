...
public class Myupdatedispatch8 extends JFrame implements ActionListener {
...
setSize(1370,800);
	   ...
	   setContentPane(contentPane);  
		...
		tt=new JTextField("       UPDATE DISPACTH");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btn2)
		{
			...
			if ( b1.length()==0 || c1.length()==0 || d1.length()==0 ||t5.getText().length()==0 || t6.getText().length()==0 || t7.getText().length()==0 )
			{
			...
			try
			{
				Class.forName("oracle.jdbc.OracleDriver");
		    	c= DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
		    	String sql="update dispatch8 set oderid=?,customerid=?,itemid=?,dispatchquantity=?,price=? where dispatch=?";
		    	st=c.prepareStatement(sql);
		    	st.setString(1, b1);
				st.setString(2, c1);
				st.setString(3, d1);
