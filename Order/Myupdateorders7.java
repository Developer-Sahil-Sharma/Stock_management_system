...
public class Myupdateorders7 extends JFrame implements ActionListener {
    ...
    Myupdateorders7()
	{
    	...
    	contentPane = new JPanel() {  
            public void paintComponent(Graphics g) {  
                 Image img = Toolkit.getDefaultToolkit().getImage(  
                           login.class.getResource("orddd.jpg"));  
       ...
       setContentPane(contentPane);  
    	...
    	k=new JLabel("Order Date :");
    	k.setBounds(400,350,600,90);
    	add(k);
        k.setFont(new Font("Impact",Font.BOLD,20));

    	k=new JLabel("Order Quantity :");
    	...
    	tt=new JTextField("       UPDATE ORDER");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource()==btn1)
		{
			...
			try
			{
				...
				String sql="select * from orders7 where odrid=?";
		...
		if(e.getSource()==btn2)
		{
			...
			try
			{
				Class.forName("oracle.jdbc.OracleDriver");
		    	c= DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
		    	String sql="update orders7 set cusid=?,itmid=?,proca=?,dateord=?,quty=? where odrid=?";
		    	st=c.prepareStatement(sql);
		    	st.setString(1, b1);
				st.setString(2, c1);
				st.setString(3, d1);              
		...
		Connection c;PreparedStatement st;ResultSet rs;
			...
			try
			{
			 Class.forName("oracle.jdbc.OracleDriver");
			 c= DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
			 String sql="select odrid from orders7";
			 st=c.prepareStatement(sql);
			 rs=st.executeQuery();
			 while(rs.next())
			 {
