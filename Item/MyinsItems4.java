...
import javax.swing.*;
public class MyinsItems4 extends JFrame implements ActionListener ,FocusListener{
	JLabel k,msg1;
	int x=0;
	...
	public MyinsItems4() {
	   ...
	   setContentPane(contentPane);  
		...
		tt=new JTextField("       Insert Items");
...
public void actionPerformed(ActionEvent e) {
	...
	if (a1.length()==0 || b1.length()==0 || c1.length()==0 || d1.length()==0 ||t5.getText().length()==0 || t6.getText().length()==0 || t7.getText().length()==0 )
	{
	...
	try
	{
	 ...
	 String sql="insert into Items4 values(?,?,?,?,?,?,?)";
...
public void focusLost(FocusEvent e) {
	if (e.getSource()==t1)
	{
		...
		if (a1.length()==0)
		{
			msg1.setText("Please Enter your Item ID ");
