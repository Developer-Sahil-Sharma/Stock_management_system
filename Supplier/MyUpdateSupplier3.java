...
public class MyUpdateSupplier3 extends JFrame implements ActionListener {

	JComboBox t1;
	
	JTextField tt,t2,t3,t4,t5,t6;
	JLabel k;
	JButton btn,btn1,btn2;
	 private JPanel contentPane;
	public MyUpdateSupplier3()
	{
setSize(1370,800);
		...
		contentPane = new JPanel() {  
	        public void paintComponent(Graphics g) {  
	             ...
	             g.drawImage(img, 0, 0, 1370, 800, this);  
	   ...
	   setContentPane(contentPane);  
		...
		t6.setBackground(Color.white);
			...
			btn.setBounds(525,550,100,30);
			add(btn);
			btn.addActionListener(this);
		    btn.setFont(new Font("Georgia",Font.BOLD,15));
		    btn.setBackground(Color.RED);
		    btn2=new JButton("Upadte");
		    ...
		    tt=new JTextField("      UPADTE SUPPLIER");
	...
	public void actionPerformed(ActionEvent e) {
		...
		try
		{
			...
			String sql="select * from supplier3 where splid=?";
		...
		if(e.getSource()==btn2)
		{
			...
			try
			{
				Class.forName("oracle.jdbc.OracleDriver");
		    	c= DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
		    	String sql="update supplier3 set splname=?,spladd=?,splcity=?,splemail=?,itmid=? where splid=?";
		    	st=c.prepareStatement(sql);
		    	st.setString(1, b1);
				st.setString(2, c1);
				st.setString(3, d1);              
	...
	Connection c;PreparedStatement st;ResultSet rs;
		...
		try
		{
		 Class.forName("oracle.jdbc.OracleDriver");
		 c= DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
		 String sql="select splid from supplier3";
		 st=c.prepareStatement(sql);
		 rs=st.executeQuery();
		 while(rs.next())
		 {
